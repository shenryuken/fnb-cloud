    ...
    @if($selectingProduct)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
            <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 border border-zinc-200 dark:border-zinc-800">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <button type="button" wire:click="$set('selectedVariantId', null)"
                                    ...
                                    <div class="flex flex-col text-left">
                                        <span class="font-semibold text-sm {{ is_null($selectedVariantId) ? 'text-pink-600 dark:text-pink-400' : 'text-zinc-800 dark:text-zinc-100' }}">Regular</span>
                                        <span class="text-xs text-zinc-400 mt-0.5">Base: RM {{ number_format($selectingProduct->price, 2) }}</span>
                                    </div>
                                    <span class="font-bold {{ is_null($selectedVariantId) ? 'text-pink-600 dark:text-pink-400' : 'text-zinc-500' }}">RM {{ number_format($selectingProduct->price, 2) }}</span>
                                ...
                                @foreach($selectingProduct->variants as $variant)
                                    <label class="relative flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-all
                                        ...
                                        <div class="flex flex-col">
                                            <span class="font-semibold text-sm {{ $selectedVariantId == $variant->id ? 'text-pink-600 dark:text-pink-400' : 'text-zinc-800 dark:text-zinc-100' }}">
                                                {{ $variant->name }}
                                            </span>
                                            <span class="text-xs text-zinc-400 mt-0.5">Base: RM {{ number_format($variant->price, 2) }}</span>
                                        </div>
                                        <span class="font-bold {{ $selectedVariantId == $variant->id ? 'text-pink-600 dark:text-pink-400' : 'text-zinc-500' }}">RM {{ number_format($variant->price, 2) }}</span>
                            ...
                            @foreach($selectingProduct->setGroups as $group)
                                <div class="space-y-3">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <span class="px-2 py-0.5 rounded-md bg-zinc-100 dark:bg-zinc-800 text-xs text-zinc-500">
                                            {{ (int) $group->min_select }}-{{ (int) $group->max_select }} required
                                        </span>
                                    </div>
                                                ...
                                                <div class="flex items-center gap-3">
                                                    @if((int) $group->max_select === 1)
                                                        <input type="radio" wire:model.live="selectedSetItems.{{ $group->id }}" value="{{ $choice->id }}"
                                                            class="w-4 h-4 border-zinc-300 dark:border-zinc-600 text-pink-500 focus:ring-pink-500/20">
                                                    @else
                                                        <input type="checkbox" wire:model.live="selectedSetItems.{{ $group->id }}" value="{{ $choice->id }}"
                                                            class="w-4 h-4 rounded border-zinc-300 dark:border-zinc-600 text-pink-500 focus:ring-pink-500/20">
                                                    @endif
                                                    <span class="font-medium text-sm text-zinc-700 dark:text-zinc-200">{{ $choice->name }}</span>
                                                </div>
                                                @if($extra > 0)
                                                    <span class="font-semibold text-sm text-pink-500">+RM {{ number_format($extra, 2) }}</span>
                                                @else
                    ...
                    @foreach($selectingProduct->addonGroups as $group)
                        <div class="space-y-3">
                            <div class="flex items-center justify-between">
                                <h4 class="text-xs font-semibold text-zinc-400 uppercase tracking-wider">{{ $group->name }}</h4>
                                <span class="px-2 py-0.5 rounded-md bg-zinc-100 dark:bg-zinc-800 text-xs text-zinc-500">
                                    {{ $group->min_select }}-{{ $group->max_select }} required
                                </span>
                            </div>
                            <div class="space-y-2">
                                @foreach($group->items as $addon)
                                    <label class="flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-all
                                        ...
                                        <span class="font-semibold text-sm text-pink-500">+RM {{ number_format($addon->price, 2) }}</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-zinc-100 dark:border-zinc-800">
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center bg-zinc-100 dark:bg-zinc-800 rounded-lg w-fit">
                                ...
                                <span class="text-xl font-bold w-12 text-center text-zinc-800 dark:text-zinc-100">{{ $quantity }}</span>
                                <button type="button" wire:click="$set('quantity', {{ $quantity + 1 }})" 
                                    class="w-10 h-10 flex items-center justify-center text-zinc-500 hover:text-pink-500 transition-colors">
                                    <flux:icon.plus class="w-4 h-4" />
                                </button>
                            </div>
                        </div>
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" wire:model.live="notes" placeholder="No spicy, extra ice..." 
                                    class="w-full rounded-lg border-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 py-2 pl-9 pr-3 text-sm focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 transition-all">
                            </div>
                            @if(count($this->quickNotes) > 0)
                                <div class="flex flex-wrap gap-1.5">
                                    @foreach($this->quickNotes as $label)
                                        <button type="button" wire:click="applyQuickNote(@js($label))"
                                            class="px-2.5 py-1 rounded-md bg-zinc-100 dark:bg-zinc-800 text-xs font-medium text-zinc-500 hover:bg-pink-50 hover:text-pink-600 dark:hover:bg-pink-900/20 transition-all">
                                            {{ $label }}
                                        </button>
                                    @endforeach
                                </div>
                ...
                <!-- Modal Footer -->
                <div class="p-4 border-t border-zinc-100 dark:border-zinc-800 flex gap-3 bg-zinc-50 dark:bg-zinc-900">
                    <button type="button" wire:click="cancelSelection" class="px-6 py-2.5 rounded-lg font-semibold text-zinc-500 text-sm hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all">
                        Cancel
                    </button>
